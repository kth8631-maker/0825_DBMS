 ...
 * - ★☆그룹 함수☆★ : N개의 값을 읽어서 1개의 결과를 반환(그릅별로 함수 결과 반환)
 */
------------------------ < 단일행 함수 > ----------------------------
SELECT
       SALARY
  ...
  FROM 
       EMPLOYEE;
-- 단일행 함수와 그룹함수는 함꼐 사용할 수 없음 : 결과 행의 개수가 다르기 때문
 ...
 * - LENGTHB(STR) : 전달된 문자열의 바이트 수 반환 (크기)
 * 
 * 결과는 NUMBER타입 (ORACLE은 정수,실수 구분없이 NUMBER타입)
 * 
 * 한글 : 'ㄱ', 'ㅏ', '강' => 한글자당 3Byte
 ...
 */
  ...
  * CEIL
  * 
  * - CEIL(NNUMBER) 소수점 아래의 수를 무조건 올림처리해주는 함수
  */      
SELECT 
       FLOOR(123.45 * 10) / 10
  FROM
 ...
 */
SELECT 
       SYSDATE -- 날짜 시간 저장공간 최적화 등 (일반적으로 많이 쓰임)
     , SYSTIMESTAMP
  FROM
       DUAL;

-- MONTHS_BETWEEN(DATE1, DATE2) : 두 날짜 사이의 개월 수 반환(NUMBER)
-- EMPLOYEE테이블로부터 각 사원 사원명, 고용일로부터 근무일수, 근무개월수 조회
SELECT 
       EMP_NAME
...
/*
 * ☆★EXTRACT : 년도 또는 월 또는 일정보를 추축해서 반환(NUMBER타입)
 * 
...
---------------------------------------------------------------------
-- 2. AVG(숫자타입) : 해당 컬럽값들의 평균값을 구해서 반환
-- 전체사원들의 급여 평균구하기
SELECT 
       ROUND(AVG(SALARY))
  FROM 
