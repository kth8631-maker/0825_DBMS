 ...
 * => 무작정 JIN을 해서 조인하는 것이 아니고 테이블 간 매칠을 할 수 있는 컬럼이 존재해야함
 * 
...
-- 3) FULL [ OUTER ] JOIN : 두테이블 가진 모든 행을 조회할 수 있는 조인
...
SELECT 
  ...
  FROM 
       EMPLOYEE
 WHERE 
       SALARY > 3000000; -- 이승철, 강병준, 강현성, 박성민, 박세혁, 박현준
       					 -- 유성현, 신국희, 8행
...
SELECT 
 ...
 WHERE
   ...
   AND
       SALARY > 3000000;
---------------------------------------------------------------
-- 4. MINUS (차집합 - 선행쿼리 결과에서 후행커리 결과를 뺀 나머지)
SELECT 
       EMP_NAME
     , DEPT_CODE
     , SALARY
