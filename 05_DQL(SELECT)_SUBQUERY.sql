 ...
 * < SUB QUERY 서브쿼리 >
 * 
 * 하나의 메인 SQL(SELECT, INSERTM UPDATE, CREATE, ...)안에 포함된
 * 또 하나의 SELECT문
 * 
...
----------------------------------------------------------------------------------
-- 부서별로 급여 합계가 가장 큰 부서의 부서명, 부서코드, 급여합계 조회
-- 1_1. 각 부서별 급여 합계
SELECT 
       SUM(SALARY)
  FROM 
...
--> JAVA
-------------------------------------------------------------------------
-- 자! 우리가 지금 하고 있는 작업이 어떤걸 하고 있는 건가요? / DB에다가 주고받고 --> 콤퓨타 작업
-------------------------------------------------------------------------
/*
 * 2. 다중 행 서브쿼리
