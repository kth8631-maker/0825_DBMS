...
-- 현재 이 계정이 가지고 있는 테이블들의 전박적인 구조를 확인할 수 있음
 ...
 * < 제약 조건 CONSTRAINT >
 * 
 * - 테이블에 유효한 데이터값만 유지하기 위해서 특정 컬럼마다 제약 == 데이터 무결섬 보장
 * - 제약조건을 부여하면 데이터를 INSERT . UPDATE할 때마다 제약조건에 위배되지 않는지 검사
 * 
 * - 종류 : NOT NULL, UNIQUEM CHECK, PRIMARY KEY, FOREIGN KEY
 * 
...
INSERT INTO TB_USER_NOT_NULL VALUES(1, 'damin', '1234', '관리자', '남');
INSERT INTO TB_USER_NOT_NULL VALUES(NULL, NULL, NULL, NULL, NULL);
-- ("KTH04"."TB_USER_NOT_NULL"."USER_NO") 안에 삽입할 수 없습니다
...
-- CHECK를 부여하더라도 NULL값은ㅑㅜㄴㄷㄳGKF TN DLtDMA -> ㅜㅐㅅ ㅜㅕㅣㅣ FH AKRDKDIGKA
-----------------------------------------------------------------------------
/*
 * 테이블 만들기 실습
 ...
 * 
 * - 테이블 레벨 방식
 * FOREIGN KEYT(컬럼명) REFERENCES 참조할테이블명(참조할컬럼명)
 * 
 ...
 * 
 * 기본 설정은 ON DELETE RESTRTIRCTED(삭제제한)이 설정됨
 */

-- 1) ON DELEETE SET NULL == 부모 데이터 삭제 시 자식레코드도 NULL값으로 변경
...
-- ALTER TABLE JOB)COPY MODIFY JOB_CODE CHAR(1);
-- 현재 변경하려고 하는 컬럼의 값과 완전히 다른 타입이거나 작은 크기러는 변경이 불가능함
-- 문자 -> 숫자(X) / 사이즈 축소(X) / 확대 (O)
SELECT * FROM JOB_COPY;

-- JOB_NAME 컬럼 데이터 타입을 NVARCHAR2(40)
-- LOCATION 컬럼 데이터 타입을 NVARCHAR(40)
-- LOCATION_NAME컬럼 기본값을 '미국'

ALTER TABLE JOB_COPY
MODIFY JOB_NAME NVARCHAR2(40)
MODIFY LOCATION NVARCHAR(40)
...
ALTER TABLE JOB _COPY2 DROP COLUMN JOB_NAME;
