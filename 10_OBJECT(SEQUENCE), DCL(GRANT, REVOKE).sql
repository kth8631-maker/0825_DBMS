 ...
 * 
 * -- 회원번호, 사원번호, 예약번호, 주문번호 등등 채번 할 때 주로 사용함
 * 
 * 1. 시퀀스 객체 생성 구문
 * 
 * [ 표현법 ]
 ...
 *           값이 필요할 때마다 매 번 새롭게 값을 생성해내는 것보다는
 *           캐시 공간에 미리 생성된 값들을 가져다 쓰는 것이 훨씬 속도적인 측면에서 이득이 있음
...
-- CURRVAL은 마지막에 성공적으로 만들어진 NEXTVAL을 보여주는 임시값

SELECT SEQ_EMPNO.NEXTVAL FROM DUAL;
SELECT * FROM USER_SQUENCES;
-- LAST_NUMBER : 지금 NEXTVAL하면 나오는 값

-- 수정 : ALTER --> START WITH는 변경 불거능 너무 바꾸고 싶으면 DROP하고 다시 CREATE
-- 삭제 : DROP
